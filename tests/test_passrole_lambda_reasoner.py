...
class TestPassedToServiceStringLikeSemantics:
    ...
    def _check_state_for(self, raw_conditions: dict[str, Any]) -> CheckState:
        ...
        finding = PassRoleLambdaReasoner().run(facts)[0]
        check_8 = next(c for c in finding.required_checks if c.name == "passrole_condition_scoped_to_lambda_or_absent")
        return check_8.state

    def test_stringequals_exact_lambda_passes(self) -> None:
        assert (
            self._check_state_for({"StringEquals": {"iam:PassedToService": "lambda.amazonaws.com"}}) is CheckState.PASS
        )

    def test_stringlike_exact_lambda_passes(self) -> None:
        assert self._check_state_for({"StringLike": {"iam:PassedToService": "lambda.amazonaws.com"}}) is CheckState.PASS

    def test_stringlike_lambda_glob_passes(self) -> None:
        assert self._check_state_for({"StringLike": {"iam:PassedToService": "lambda.*"}}) is CheckState.PASS
    ...
    def test_stringlike_wrong_service_fails(self) -> None:
        assert self._check_state_for({"StringLike": {"iam:PassedToService": "ec2.*"}}) is CheckState.FAIL
    ...
    def test_unsupported_operator_is_unknown(self) -> None:
        assert (
            self._check_state_for({"StringNotEquals": {"iam:PassedToService": "lambda.amazonaws.com"}})
            is CheckState.UNKNOWN
        )

    def test_stringlike_list_passes_when_any_value_matches(self) -> None:
        assert self._check_state_for({"StringLike": {"iam:PassedToService": ["ec2.*", "lambda.*"]}}) is CheckState.PASS
