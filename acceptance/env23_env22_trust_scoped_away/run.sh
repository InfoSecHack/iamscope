...
OUTPUT_DIR="${OUTPUT_DIR:-/tmp/env23-cross-account-output}"
COLLECTION_ROLE_NAME="${COLLECTION_ROLE_NAME:-env23-iamscope-reader}"
...
if [[ "${CONFIRM_ENV23_CROSS_ACCOUNT_MUTATION:-}" != "YES" ]]; then
  echo "FAIL: set CONFIRM_ENV23_CROSS_ACCOUNT_MUTATION=YES to run Env23 Terraform setup" >&2
  exit 2
fi
...
echo "--- Env23 cross-account preflight ---"
...
aws --profile "$MANAGEMENT_PROFILE" --region "$AWS_REGION" sts assume-role \
  ...
  --role-session-name env23-caller-precollect-check \
  --output json >/dev/null
aws --profile "$MANAGEMENT_PROFILE" --region "$AWS_REGION" sts assume-role \
  --role-arn "$TARGET_COLLECTION_ROLE_ARN" \
  --role-session-name env23-target-precollect-check \
  --output json >/dev/null
