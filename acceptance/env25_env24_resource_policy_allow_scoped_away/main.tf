...
provider "aws" {
  ...
  default_tags {
    tags = {
      ManagedBy    = "terraform"
      Purpose      = "iamscope-env25-acceptance"
      iamscope-env = "env25"
    }
  }
}
...
locals {
  bucket_name = "env25-rp-scoped-away-${data.aws_caller_identity.current.account_id}-${random_id.suffix.hex}"
