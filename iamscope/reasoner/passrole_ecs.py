"""PassRole-to-ECS privilege escalation reasoner — S12 (flagship).

The flagship reasoner of the rebuild. Implements the full 8-check logic
from plan §4B for the canonical ECS privilege escalation pattern:

    A principal with `ecs:RegisterTaskDefinition` + `ecs:RunTask` AND `iam:PassRole`
    targeting a role that trusts `ecs-tasks.amazonaws.com` can:
        1. create a ECS task
...
3. **It is the reasoner FindingsForge will consume on day one** for
   the flagship ECS privilege escalation pattern.
...
**Candidate enumeration is target-first** (find ECS-trusting roles,
then find principals with ecs:RegisterTaskDefinition + ecs:RunTask, then pair them) rather
...
fixture F because the target role IS in the graph (it trusts ECS tasks)
and Alice IS in the graph (she has ecs:RegisterTaskDefinition + ecs:RunTask); the
...
# Service principal for ECS execution. Hardcoded as the canonical
# AWS service principal name; will not change.
_ECS_TASKS_SERVICE_PRINCIPAL: str = "ecs-tasks.amazonaws.com"

# The two actions whose presence on the source principal defines the
# ECS PassRole pattern.
_ECS_REGISTER_TASK_DEF_ACTION: str = "ecs:RegisterTaskDefinition"
_ECS_RUN_TASK_ACTION: str = "ecs:RunTask"
...
class PassRoleEcsReasoner:
    """Reasoner for the ECS PassRole privilege escalation chain (§4B).

    Identity attributes per Reasoner Protocol:
    """

    pattern_id: str = "passrole_ecs"
    pattern_version: str = "1.0.0"
    pattern_title: str = "ECS Task RunTask + PassRole Privilege Chain"
    ...
    def preconditions_met(self, facts: FactGraph) -> tuple[bool, str]:
        ...
        if sample is not None and "raw_conditions" not in sample.features:
            return (
                False,
                "PR-1 not applied: permission edges lack raw_conditions; "
                "passrole_ecs refuses to run against pre-fix collector",
            )
    ...
    def _enumerate_candidates(
        ...
        Target-first: for each IAMRole that trusts ECS tasks, find every
        IAMUser / IAMRole in the graph that has ecs:RegisterTaskDefinition + ecs:RunTask
        (`has_action(...)` returning anything other than FAIL).
        Pair them. Pairs where the source is the target itself are
        skipped (a role passing itself to ECS is not the standard
        privilege escalation chain).
        """
        ecs_trusting_roles: list[Node] = []
        for node in facts.nodes:
            if node.node_type != NODE_TYPE_IAM_ROLE:
                continue
            if self._role_trusts_ecs_tasks(facts, node.provider_id):
                ecs_trusting_roles.append(node)

        if not ecs_trusting_roles:
            return []

        # Find every principal with any ECS-compute permission (either
        # RegisterTaskDefinition OR RunTask). Candidate filter is
        # inclusive — the per-candidate evaluation applies the strict
        # AND check on both actions. `has_action` is the tristate API;
        # we accept PASS or UNKNOWN (UNKNOWN means there IS a matching
        # edge but with ambiguity flags — still a candidate worth
        # evaluating, will produce an inconclusive finding).
        principals_with_ecs_compute: list[Node] = []
        for node in facts.nodes:
            if node.node_type not in (NODE_TYPE_IAM_USER, NODE_TYPE_IAM_ROLE):
                continue
            state_register = facts.has_action(
                node.provider_id,
                _ECS_REGISTER_TASK_DEF_ACTION,
            )
            state_run = facts.has_action(
                node.provider_id,
                _ECS_RUN_TASK_ACTION,
            )
            if state_register is not CheckState.FAIL and state_run is not CheckState.FAIL:
                principals_with_ecs_compute.append(node)

        # Cartesian product, deduplicated, with self-pairs filtered.
        candidates: list[tuple[Node, Node]] = []
        seen: set[tuple[str, str]] = set()
        for source in principals_with_ecs_compute:
            for target in ecs_trusting_roles:
    ...
    def _role_trusts_ecs_tasks(self, facts: FactGraph, role_arn: str) -> bool:
        """Does this role's trust policy trust ecs-tasks.amazonaws.com?

        Walks `trust_policy_of(role_arn)` (which returns trust edges TO
        the role) and looks for any edge whose src is an AWSService
        node with provider_id == "ecs-tasks.amazonaws.com" and effect
        ...
        for edge in facts.trust_policy_of(role_arn):
            if (
                ...
                and edge.src.provider_id == _ECS_TASKS_SERVICE_PRINCIPAL
    ...
    def _evaluate_candidate(
        ...
        # ---- Check 1: source has BOTH ecs:RegisterTaskDefinition AND ecs:RunTask
        # Per plan: ECS privilege escalation requires a principal that
        # can (a) register a task definition naming the target role,
        # and (b) run tasks from that definition. Either alone is
        # insufficient — RegisterTaskDefinition without RunTask means
        # you can create malicious task defs but can't execute them,
        # and RunTask without RegisterTaskDefinition means you can
        # only run existing task defs, which limits what code runs.
        #
        # Combined via _and_tristate:
        #   PASS if both PASS
        #   FAIL if either FAIL (early exit, no finding)
        #   UNKNOWN if neither FAIL but at least one UNKNOWN
        check_1a_state = facts.has_action(source_arn, _ECS_REGISTER_TASK_DEF_ACTION)
        check_1b_state = facts.has_action(source_arn, _ECS_RUN_TASK_ACTION)
        check_1_state = self._and_tristate(check_1a_state, check_1b_state)
        if check_1_state is CheckState.FAIL:
            return None  # early exit, missing at least one required action
        ...
        # Find witnesses for BOTH ECS actions. Used separately by the
        # SCP/boundary checks (4 and 6) to verify blockers on EACH
        # required action — the strictest interpretation: the chain is
        # only validated if neither action is blocked on its respective
        # witness edge. The PRIMARY witness (used for check 1's
        # evidence_refs) is the RegisterTaskDefinition witness when
        # present, else the RunTask witness.
        register_witness = self._find_witness_edge(
            facts,
            source_arn,
            _ECS_REGISTER_TASK_DEF_ACTION,
            resource_pattern=None,
        )
        run_witness = self._find_witness_edge(
            facts,
            source_arn,
            _ECS_RUN_TASK_ACTION,
            resource_pattern=None,
        )
        check_1_witness = register_witness or run_witness

        # If neither witness was found despite check 1 not being FAIL,
        # something is wrong with the fact graph internal consistency —
        # bail to inconclusive via UNKNOWN.
        ...
        # (DIG-1 post-S05). Include BOTH ECS witnesses (register + run)
        # plus the PassRole witness so every relevant statement is
        # cited in evidence.
        for witness in (register_witness, run_witness, check_2_witness):
            if witness is None:
                continue
            if witness.edge_id not in edge_refs:
        ...
        # ---- Check 1 result row.
        # evidence_refs cites BOTH witnesses when present so the
        # reviewer can inspect both action grants.
        check_1_refs: tuple[str, ...] = tuple(w.edge_id for w in (register_witness, run_witness) if w is not None) or (
            source_arn,
        )
        check_results.append(
            Check(
                name="source_has_ecs_create_and_run_permissions",
                description="Source principal has ecs:RegisterTaskDefinition AND ecs:RunTask",
                state=check_1_state,
                evidence_refs=check_1_refs,
                reason=self._explain_ecs_check_1(
                    check_1a_state,
                    check_1b_state,
                ),
            )
        ...
        trace.append(
            TraceEntry(
                step=1,
                action="check_source_has_ecs_create_and_run_permissions",
                inputs=(source_arn, _ECS_REGISTER_TASK_DEF_ACTION, _ECS_RUN_TASK_ACTION),
                result=check_1_state.value.upper(),
                reason=self._explain_ecs_check_1(check_1a_state, check_1b_state),
            )
        )
        ...
        # ---- Check 3: target trusts ECS tasks.
        ...
        ecs_trust_edge = self._find_ecs_trust_edge(facts, target_arn)
        if ecs_trust_edge is not None:
            check_3_state = CheckState.PASS
            check_3_reason = (
                f"target role {target_arn} trusts ecs-tasks.amazonaws.com via trust edge {ecs_trust_edge.edge_id}"
            )
            edge_refs.append(ecs_trust_edge.edge_id)
            # Pull trust statement digests too.
            for ref in (ecs_trust_edge.features or {}).get("allow_controls", []) or []:
                if isinstance(ref, dict):
                    digest = ref.get("digest", "")
                    if digest:
                        statement_digests.add(digest)
            ...
            check_3_reason = f"target role {target_arn} does not trust ecs-tasks.amazonaws.com"
        check_results.append(
            Check(
                name="target_trusts_ecs_tasks_service",
                description="Target role's trust policy trusts ECS tasks",
                state=check_3_state,
                evidence_refs=((ecs_trust_edge.edge_id,) if ecs_trust_edge is not None else ()),
        ...
        trace.append(
            TraceEntry(
                step=3,
                action="check_target_trusts_ecs_tasks_service",
                inputs=(target_arn, _ECS_TASKS_SERVICE_PRINCIPAL),
                result=check_3_state.value.upper(),
                reason=check_3_reason,
            )
        )

        # ---- Check 4: no SCP blocks EITHER ECS action.
        # ECS privilege escalation requires BOTH RegisterTaskDefinition
        # and RunTask to succeed. An SCP blocking either action with
        # complete confidence is sufficient to break the chain, so we
        # run _check_scp_blockers against both witnesses and combine
        # via _and_tristate. The refuses-to-lie property holds: if
        # either witness lookup returns UNKNOWN (partial/needs_review
        # confidence), the combined state is UNKNOWN and the verdict
        # drops to inconclusive.
        check_4a_state, check_4a_reason, check_4a_blockers = self._check_scp_blockers(
            facts,
            register_witness,
            constraint_refs,
            edge_constraint_refs,
            action_label="ecs:RegisterTaskDefinition",
        )
        check_4b_state, check_4b_reason, check_4b_blockers = self._check_scp_blockers(
            facts,
            run_witness,
            constraint_refs,
            edge_constraint_refs,
            action_label="ecs:RunTask",
        )
        check_4_state = self._and_tristate(check_4a_state, check_4b_state)
        check_4_reason = f"RegisterTaskDefinition: {check_4a_reason}; RunTask: {check_4b_reason}"
        blockers.extend(check_4a_blockers + check_4b_blockers)
        check_results.append(
            Check(
                name="no_scp_blocks_ecs_create_or_run",
                description="No SCP blocks ecs:RegisterTaskDefinition or ecs:RunTask with full confidence",
                state=check_4_state,
        ...
        trace.append(
            TraceEntry(
                step=4,
                action="check_no_scp_blocks_ecs_create_or_run",
                inputs=tuple(w.edge_id for w in (register_witness, run_witness) if w is not None),
        ...
        # ---- Check 6: no boundary blocks EITHER ECS action.
        # Same two-witness logic as check 4 — boundaries blocking
        # either RegisterTaskDefinition or RunTask break the chain.
        bnd_6_constraint_refs: set[str] = set()
        check_6a_state, check_6a_reason, check_6a_blockers = self._check_boundary_blockers(
            facts,
            register_witness,
        ...
        check_6b_state, check_6b_reason, check_6b_blockers = self._check_boundary_blockers(
            facts,
            run_witness,
            bnd_6_constraint_refs,
            edge_constraint_refs,
        ...
        check_6_state = self._and_tristate(check_6a_state, check_6b_state)
        check_6_reason = f"RegisterTaskDefinition: {check_6a_reason}; RunTask: {check_6b_reason}"
        constraint_refs.update(bnd_6_constraint_refs)
        blockers.extend(check_6a_blockers + check_6b_blockers)
        check_results.append(
            Check(
                name="no_boundary_blocks_ecs_create_or_run",
                description="Permission boundary on source allows both ecs:RegisterTaskDefinition and ecs:RunTask",
                state=check_6_state,
        ...
        trace.append(
            TraceEntry(
                step=6,
                action="check_no_boundary_blocks_ecs_create_or_run",
                inputs=tuple(w.edge_id for w in (register_witness, run_witness) if w is not None),
        ...
        check_8_state, check_8_reason = self._check_passed_to_service(
            check_2_witness,
        )
        check_results.append(
            Check(
                name="passrole_condition_scoped_to_ecs_or_absent",
                description=("iam:PassRole condition iam:PassedToService is absent or scoped to ECS tasks"),
                state=check_8_state,
                evidence_refs=((check_2_witness.edge_id,) if check_2_witness is not None else ()),
        ...
        trace.append(
            TraceEntry(
                step=8,
                action="check_passrole_condition_scoped_to_ecs_or_absent",
                inputs=(check_2_witness.edge_id if check_2_witness else "",),
        ...
        # ---- Admin-equivalence read (separate from the 8 checks).
        ...
        if check_8_state is CheckState.FAIL:
            blockers.append(
                Blocker(
        ...
        if verdict is Verdict.VALIDATED:
            assumptions.append(
                Assumption(
                    kind="session_policy",
                    detail=(
                        "no session policy restricts ecs:RegisterTaskDefinition, ecs:RunTask, "
    ...
    @staticmethod
    def _and_tristate(a: CheckState, b: CheckState) -> CheckState:
        """Delegate to the shared combinator (priority 3b prep refactor).

        Originally defined as a staticmethod here for the ECS two-action
        check 1 logic. Promoted to `iamscope.reasoner.combinators` so
        future multi-action and multi-hop reasoners (assume_role_chain,
        secrets blast radius) can import it without copy-pasting. This
        method survives as a thin delegate to keep call sites stable.
        """
        from iamscope.reasoner.combinators import and_tristate

        return and_tristate(a, b)
    ...
    def _find_ecs_trust_edge(
        self,
        facts: FactGraph,
        role_arn: str,
    ) -> Edge | None:
        """Find the trust edge proving the role trusts ECS tasks."""
        for edge in facts.trust_policy_of(role_arn):
            if (
                edge.src.node_type == NODE_TYPE_AWS_SERVICE
                and edge.src.provider_id == _ECS_TASKS_SERVICE_PRINCIPAL
    ...
    def _check_passed_to_service(
        ...
        Per plan §4B.2 row 8:
        - PASS: condition absent OR present and value includes ecs-tasks.amazonaws.com
        - FAIL: condition present and value does NOT include ECS (e.g. ec2)
        ...
        # - scoped to ECS (with or without other services) → PASS
        # - scoped to other services only → FAIL
        if scoped_to_ecs_tasks:
            return (
                CheckState.PASS,
                f"iam:PassedToService includes {_ECS_TASKS_SERVICE_PRINCIPAL}",
            )
        return (
            CheckState.FAIL,
    ...
    def _is_admin_equivalent(
        ...
        """Delegate to shared admin detection module (3c-refactor).

        See passrole_lambda's helper docstring for the full history.
        """
    ...
    def _compute_verdict_and_severity(
        ...
        check_3 = check_by_name.get("target_trusts_ecs_tasks_service")
        check_4 = check_by_name.get("no_scp_blocks_ecs_create_or_run")
        check_5 = check_by_name.get("no_scp_blocks_passrole")
        check_6 = check_by_name.get("no_boundary_blocks_ecs_create_or_run")
        check_7 = check_by_name.get("no_boundary_blocks_passrole")
        check_8 = check_by_name.get("passrole_condition_scoped_to_ecs_or_absent")
        ...
        if check_3 is not None and check_3.state is CheckState.FAIL:
            return (
                ...
                SEVERITY_MEDIUM,
                "target role does not trust ECS — chain not exploitable as written",
            )

        # Rule 4: Check 8 FAIL → precondition_only / medium.
        if check_8 is not None and check_8.state is CheckState.FAIL:
            return (
                Verdict.PRECONDITION_ONLY,
                SEVERITY_MEDIUM,
                "iam:PassedToService scoped away from ECS — chain not exploitable",
            )
        ...
        if check_4 is not None and check_4.state is CheckState.FAIL:
            return (
                Verdict.BLOCKED,
                SEVERITY_INFO,
                "SCP blocks ecs:RegisterTaskDefinition or ecs:RunTask with complete confidence",
            )
        ...
        if check_6 is not None and check_6.state is CheckState.FAIL:
            return (
                Verdict.BLOCKED,
                SEVERITY_INFO,
                "permission boundary blocks ecs:RegisterTaskDefinition or ecs:RunTask (post-BND-1)",
            )
        ...
        # Rule 7: Any check UNKNOWN → inconclusive / high.
        ...
        # Rule 8/9: All PASS → validated.
        if is_admin:
            return (
                Verdict.VALIDATED,
                SEVERITY_CRITICAL,
                "all 8 checks PASS; target role has admin-equivalent permissions",
            )
        return (
            Verdict.VALIDATED,
            SEVERITY_HIGH,
            "all 8 checks PASS; non-admin target role",
        )
    ...
    def _explain_ecs_check_1(
        self,
        register_state: CheckState,
        run_state: CheckState,
    ) -> str:
        """Build a reason string for check 1's combined two-action state.

        Check 1 is the AND-combined state over ecs:RegisterTaskDefinition
        and ecs:RunTask. The reason string must tell a reviewer which of
        the two actions failed or was ambiguous — a flat "check failed"
        message would lose the per-action detail.

        Semantics mirror `_explain_check_state` for consistency:
            - both PASS: "both actions have explicit non-conditioned edges"
            - either FAIL: "no matching edge for <action>"
            - either UNKNOWN: "matching <action> edge has ambiguity flag"
        """
        r_label = "ecs:RegisterTaskDefinition"
        n_label = "ecs:RunTask"

        if register_state is CheckState.FAIL:
            return f"no matching {r_label} edge"
        if run_state is CheckState.FAIL:
            return f"no matching {n_label} edge"

        if register_state is CheckState.PASS and run_state is CheckState.PASS:
            return f"both {r_label} and {n_label} have explicit non-conditioned permission edges"

        # At least one is UNKNOWN, neither is FAIL.
        unknown_actions: list[str] = []
        if register_state is CheckState.UNKNOWN:
            unknown_actions.append(r_label)
        if run_state is CheckState.UNKNOWN:
            unknown_actions.append(n_label)
        labels = " and ".join(unknown_actions)
        return f"matching edge for {labels} has ambiguity flag (hyperedge, wildcard resource, or conditions)"
    ...
    def _compose_title(
        ...
        if verdict is Verdict.VALIDATED:
            return f"{source_arn} can assume{admin_qualifier} role {target_arn} via ECS PassRole chain"
        if verdict is Verdict.BLOCKED:
            return f"Blocked ECS PassRole chain from {source_arn} to {target_arn}"
        if verdict is Verdict.PRECONDITION_ONLY:
            return f"Precondition-only ECS PassRole chain from {source_arn} to {target_arn}"
        return f"Inconclusive ECS PassRole chain from {source_arn} to {target_arn}"
