# Phase 0 Benchmark Snapshot: phase0-20260509-env27

- snapshot_id: `phase0-20260509-env27`
- created_at: `2026-05-09T21:31:33Z`
- source runs dir: `/tmp/iamscope-env27-freeze-runs`
- source corpus dir: `/tmp/iamscope-env27-freeze-corpus`
- corpus decision: `hold_review`
- total evaluated cases: `24`
- passed cases: `24`
- failed cases: `0`

## Env26 / Env27 Multi-Hop Evidence

- Env26 is included as `env26_multihop_chain_validated_admin` from
  `iamscope-benchmark-env26-20260509T012216Z`.
- Env27 is included as `env27_multihop_trust_scoped_away_nonvalidated` from
  `iamscope-benchmark-env27-20260509T212354Z`.
- Env26 proves one controlled same-account 3-hop AssumeRole chain validates.
- Env27 preserves that same-account multihop shape while scoping the middle
  hop trust to `env27-decoy` instead of `env27-hop1`.
- Env27 proves the first-hop permission/trust evidence, hop1-to-hop2
  permission evidence, decoy-to-hop2 trust evidence, downstream hop2-to-admin
  evidence, and zero validated Alice-to-admin `assume_role_chain` /
  `admin_reachability` findings.

## Evidence Boundary

- Env26/Env27 prove one controlled same-account multihop pair only.
- This snapshot does not claim arbitrary enterprise graph correctness,
  cross-account multihop behavior, deeper-chain behavior, SCP behavior,
  permission-boundary behavior, identity Deny behavior, condition handling,
  runtime exploitability, or production readiness.

## Artifact Hygiene

- This snapshot stores evaluated benchmark artifacts only: run manifests,
  scorer results, gate results, per-run reports, and corpus summary/report
  files.
- It intentionally does not copy raw AWS archives, Terraform state/cache or
  provider files, collect directories, `scenario.json`, `findings.json`,
...
- env26_multihop_chain_validated_admin / iamscope-benchmark-env26-20260509T012216Z -> runs/env26-20260509T012216Z
...
- env27_multihop_trust_scoped_away_nonvalidated / iamscope-benchmark-env27-20260509T212354Z -> runs/env27-20260509T212354Z
