# Phase 0 Benchmark Snapshot: phase0-20260509-env26

- snapshot_id: `phase0-20260509-env26`
- created_at: `2026-05-09T01:40:04Z`
- source runs dir: `/tmp/iamscope-env26-freeze-runs`
- source corpus dir: `/tmp/iamscope-env26-freeze-corpus`
- corpus decision: `hold_review`
- total evaluated cases: `23`
- passed cases: `23`
- failed cases: `0`
...
## Env26 Multi-Hop Chain Evidence

- `env26_multihop_chain_validated_admin` is included from `iamscope-benchmark-env26-20260509T012216Z`.
- Env26 records one Alice IAMUser node, one hop1 IAMRole node, one hop2 IAMRole node, and one admin IAMRole node.
- Env26 records all three exact `sts:AssumeRole_permission` edges for `env26-alice -> env26-hop1 -> env26-hop2 -> env26-admin`.
- Env26 records all three exact `sts:AssumeRole_trust` edges for the same path.
- Env26 records one validated `assume_role_chain` finding for the Alice-to-admin path.
- Env26 records one validated `admin_reachability` finding for the Alice-to-admin path.
- Env26 records zero blocked, inconclusive, or blocker-bearing validated target findings for both target reasoners.

## Evidence Boundary

- Env26 proves one controlled same-account 3-hop AssumeRole chain only.
- This snapshot does not claim arbitrary enterprise graph correctness, chains deeper than the current reasoner depth cap, cross-account multi-hop behavior, SCP behavior, boundary behavior, identity Deny behavior, condition handling, runtime exploitability, or production readiness.
- This snapshot is bounded live AWS benchmark evidence only.
