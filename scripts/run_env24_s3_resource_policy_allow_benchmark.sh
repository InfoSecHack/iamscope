...
ENV_DIR="$PROJECT_ROOT/acceptance/env24_s3_resource_policy_allow"
EXPECTED_JSON="$ENV_DIR/expected_findings.json"
RUN_ID="${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)}"
WORK_DIR="${IAMSCOPE_ENV24_WORKDIR:-/tmp/iamscope-env24-benchmark-$RUN_ID}"
ARTIFACT_DIR="${IAMSCOPE_BENCHMARK_OUT:-/tmp/iamscope-benchmark-env24-$RUN_ID}"
...
if [[ "$raw_rc" =~ ^[0-9]+$ ]]; then
  ...
  echo "WARNING: non-numeric exit code from Env24 run: [$raw_rc]" | tee -a "$RUN_LOG" >&2
fi
...
if [[ ! -f "$SCENARIO_JSON" || ! -f "$FINDINGS_JSON" ]]; then
  ...
  if [[ -z "$reader_arn" || -z "$bucket_arn" ]]; then
    echo "benchmark semantic assertion: FAIL (missing reader_arn/bucket_arn in $RUN_LOG)" | tee -a "$RUN_LOG"
    benchmark_rc=1
  else
    rp_edge_type=$(jq -r '.benchmark_contract.resource_policy_allow_edge.edge_type' "$EXPECTED_JSON")
    rp_min_count=$(jq -r '.benchmark_contract.resource_policy_allow_edge.min_count' "$EXPECTED_JSON")
    rp_permission_source=$(jq -r '.benchmark_contract.resource_policy_allow_edge.permission_source' "$EXPECTED_JSON")
    rp_layer=$(jq -r '.benchmark_contract.resource_policy_allow_edge.layer' "$EXPECTED_JSON")
    identity_edge_type=$(jq -r '.benchmark_contract.identity_permission_edge.edge_type' "$EXPECTED_JSON")
    identity_expected_count=$(jq -r '.benchmark_contract.identity_permission_edge.expected_count' "$EXPECTED_JSON")
    ...
    rp_edge_count=$(jq --arg typ "$rp_edge_type" --arg src "$reader_arn" --arg tgt "$bucket_arn" --arg permission_source "$rp_permission_source" --arg layer "$rp_layer" '[.edges[] | select(
        .edge_type == $typ
    ...
    identity_edge_count=$(jq --arg typ "$identity_edge_type" --arg src "$reader_arn" --arg tgt "$bucket_arn" '[.edges[] | select(.edge_type == $typ and .src.provider_id == $src and .dst.provider_id == $tgt)] | length' "$SCENARIO_JSON")
    ...
    echo "semantic assertion counts: scenario.reader_nodes=$reader_node_count scenario.bucket_nodes=$bucket_node_count scenario.resource_policy_edges=$rp_edge_count scenario.identity_permission_edges=$identity_edge_count scenario.resource_policy_condition_constraints=$condition_constraint_count scenario.resource_policy_deny_constraints=$deny_constraint_count scenario.validation=$scenario_validation_ok" | tee -a "$RUN_LOG"

    if (( reader_node_count >= 1 && bucket_node_count >= 1 && rp_edge_count >= rp_min_count && identity_edge_count == identity_expected_count && condition_constraint_count == condition_constraints_expected && deny_constraint_count == deny_constraints_expected )); then
...
cat <<EOF
Env24 benchmark pass finished with command exit code: $cmd_rc
Env24 benchmark final result code: $benchmark_rc
