...
ENV_DIR="$PROJECT_ROOT/acceptance/env08_trust_condition_blocked_admin"
EXPECTED_JSON="$ENV_DIR/expected_findings.json"
RUN_ID="${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)}"
WORK_DIR="${IAMSCOPE_ENV08_WORKDIR:-/tmp/iamscope-env08-benchmark-$RUN_ID}"
ARTIFACT_DIR="${IAMSCOPE_BENCHMARK_OUT:-/tmp/iamscope-benchmark-env08-$RUN_ID}"
...
if [[ "$raw_rc" =~ ^[0-9]+$ ]]; then
  ...
  echo "WARNING: non-numeric exit code from Env08 run: [$raw_rc]" | tee -a "$RUN_LOG" >&2
fi
...
if [[ ! -f "$SCENARIO_JSON" || ! -f "$FINDINGS_JSON" ]]; then
  ...
  target_arn=$(sed -n 's/^  conditioned_admin_arn: //p' "$RUN_LOG" | tail -1)
  if [[ -z "$source_arn" || -z "$target_arn" ]]; then
    echo "benchmark semantic assertion: FAIL (missing alice_arn/conditioned_admin_arn in $RUN_LOG)" | tee -a "$RUN_LOG"
    benchmark_rc=1
  else
    ...
    arc_inconclusive_count=$(jq --arg src "$source_arn" --arg tgt "$target_arn" '[.findings[] | select(.pattern_id == "assume_role_chain" and .source.provider_id == $src and .target.provider_id == $tgt and .verdict == "inconclusive")] | length' "$FINDINGS_JSON")

    echo "semantic assertion counts: scenario.permission_edges=$perm_edge_count scenario.trust_edges=$trust_edge_count scenario.trust_condition_constraints=$trust_condition_count scenario.trust_condition_bindings=$trust_binding_count admin_reachability.validated=$ar_validated_count admin_reachability.blocked=$ar_blocked_count admin_reachability.inconclusive=$ar_inconclusive_count assume_role_chain.validated=$arc_validated_count assume_role_chain.inconclusive=$arc_inconclusive_count scenario.validation=$scenario_validation_ok" | tee -a "$RUN_LOG"
...
cat <<EOF
Env08 benchmark pass finished with command exit code: $cmd_rc
Env08 benchmark final result code: $benchmark_rc
