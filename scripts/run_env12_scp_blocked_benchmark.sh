...
ENV_DIR="$PROJECT_ROOT/acceptance/env12_scp_blocked_assumerole"
EXPECTED_JSON="$ENV_DIR/expected_findings.json"
RUN_ID="${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)}"
WORK_DIR="${IAMSCOPE_ENV12_WORKDIR:-/tmp/iamscope-env12-benchmark-$RUN_ID}"
ARTIFACT_DIR="${IAMSCOPE_BENCHMARK_OUT:-/tmp/iamscope-benchmark-env12-$RUN_ID}"
...
if [[ "$raw_rc" =~ ^[0-9]+$ ]]; then
  ...
  echo "WARNING: non-numeric exit code from Env12 run: [$raw_rc]" | tee -a "$RUN_LOG" >&2
fi
...
if [[ ! -f "$SCENARIO_JSON" || ! -f "$FINDINGS_JSON" || ! -f "$BINDING_METADATA_JSON" ]]; then
  ...
  if [[ -z "$source_arn" || -z "$target_arn" ]]; then
    ...
    scp_constraint_ids_json=$(jq -c --arg ctype "$scp_constraint_type" --arg action "$scp_deny_action" '[.constraints[] | select(.constraint_type == $ctype and (.properties.deny_actions // [] | index($action))) | .constraint_id]' "$SCENARIO_JSON")
    scp_constraint_count=$(jq --arg ctype "$scp_constraint_type" --arg action "$scp_deny_action" '[.constraints[] | select(.constraint_type == $ctype and (.properties.deny_actions // [] | index($action)))] | length' "$SCENARIO_JSON")
    ...
    arc_blocked_count=$(jq '[.findings[] | select(.pattern_id == "assume_role_chain" and .verdict == "blocked")] | length' "$FINDINGS_JSON")
    arc_inconclusive_count=$(jq '[.findings[] | select(.pattern_id == "assume_role_chain" and .verdict == "inconclusive")] | length' "$FINDINGS_JSON")

    echo "semantic assertion counts: scenario.accounts_collected=$accounts_collected scenario.permission_edges=$perm_edge_count scenario.trust_edges=$trust_edge_count scenario.scp_constraints=$scp_constraint_count scenario.scp_bindings=$scp_binding_count binding_metadata.scp_complete_blocking=$scp_complete_blocking_count admin_reachability.validated=$ar_validated_count admin_reachability.blocked=$ar_blocked_count admin_reachability.inconclusive=$ar_inconclusive_count assume_role_chain.validated=$arc_validated_count assume_role_chain.blocked=$arc_blocked_count assume_role_chain.inconclusive=$arc_inconclusive_count scenario.validation=$scenario_validation_ok" | tee -a "$RUN_LOG"

    if (( scenario_validation_ok == 1 && accounts_collected > 0 && perm_edge_count >= min_edge_count && trust_edge_count >= min_edge_count && scp_constraint_count >= min_scp_constraints && scp_binding_count >= min_scp_bindings && ar_validated_count <= ar_validated_max && arc_validated_count <= arc_validated_max )); then
...
cat <<EOF
Env12 benchmark pass finished with command exit code: $cmd_rc
Env12 benchmark final result code: $benchmark_rc
