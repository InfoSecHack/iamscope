...
ENV_DIR="$PROJECT_ROOT/acceptance/env15_env14_permission_condition_removed"
EXPECTED_JSON="$ENV_DIR/expected_findings.json"
RUN_ID="${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)}"
WORK_DIR="${IAMSCOPE_ENV15_WORKDIR:-/tmp/iamscope-env15-benchmark-$RUN_ID}"
ARTIFACT_DIR="${IAMSCOPE_BENCHMARK_OUT:-/tmp/iamscope-benchmark-env15-$RUN_ID}"
...
if [[ "$raw_rc" =~ ^[0-9]+$ ]]; then
  ...
  echo "WARNING: non-numeric exit code from Env15 run: [$raw_rc]" | tee -a "$RUN_LOG" >&2
fi
...
if [[ ! -f "$SCENARIO_JSON" || ! -f "$FINDINGS_JSON" ]]; then
  ...
  if [[ -z "$source_arn" || -z "$target_arn" ]]; then
    ...
    permission_condition_key=$(jq -r '.benchmark_contract.forbidden_permission_condition.condition_key' "$EXPECTED_JSON")
    max_condition_edges=$(jq -r '.benchmark_contract.forbidden_permission_condition.max_edges' "$EXPECTED_JSON")
    ...
    permission_condition_edge_count=$(jq --arg typ "$perm_edge_type" --arg src "$source_arn" --arg tgt "$target_arn" --arg ckey "$permission_condition_key" '[.edges[] | select(
        ...
        and ((.features.has_conditions == true) or (((.features.raw_conditions // {}) | tostring) | contains($ckey)))
    ...
    echo "semantic assertion counts: scenario.permission_edges=$perm_edge_count scenario.trust_edges=$trust_edge_count permission_condition_edges=$permission_condition_edge_count admin_reachability.validated=$ar_validated_count admin_reachability.blocked=$ar_blocked_count admin_reachability.inconclusive=$ar_inconclusive_count validated_with_blockers=$validated_with_blockers_count scenario.validation=$scenario_validation_ok" | tee -a "$RUN_LOG"

    if (( perm_edge_count >= min_edge_count && trust_edge_count >= min_edge_count && permission_condition_edge_count <= max_condition_edges && ar_validated_count >= ar_validated_min && ar_blocked_count == ar_blocked_expected && ar_inconclusive_count == ar_inconclusive_expected && validated_with_blockers_count == validated_with_blockers_expected )); then
...
cat <<EOF
Env15 benchmark pass finished with command exit code: $cmd_rc
Env15 benchmark final result code: $benchmark_rc
