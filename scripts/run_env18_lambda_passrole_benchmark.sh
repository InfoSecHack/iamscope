...
ENV_DIR="$PROJECT_ROOT/acceptance/env18_lambda_passrole_validated"
EXPECTED_JSON="$ENV_DIR/expected_findings.json"
RUN_ID="${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)}"
WORK_DIR="${IAMSCOPE_ENV18_WORKDIR:-/tmp/iamscope-env18-benchmark-$RUN_ID}"
ARTIFACT_DIR="${IAMSCOPE_BENCHMARK_OUT:-/tmp/iamscope-benchmark-env18-$RUN_ID}"
...
if [[ "$raw_rc" =~ ^[0-9]+$ ]]; then
  ...
  echo "WARNING: non-numeric exit code from Env18 run: [$raw_rc]" | tee -a "$RUN_LOG" >&2
fi
...
if [[ ! -f "$SCENARIO_JSON" || ! -f "$FINDINGS_JSON" ]]; then
  ...
  if [[ -z "$source_arn" || -z "$target_arn" ]]; then
    ...
    validated_min=$(jq -r '.benchmark_contract.passrole_lambda.validated_min' "$EXPECTED_JSON")
    ...
    precondition_only_expected=$(jq -r '.benchmark_contract.passrole_lambda.precondition_only' "$EXPECTED_JSON")
    validated_with_blockers_expected=$(jq -r '.benchmark_contract.passrole_lambda.validated_with_blockers' "$EXPECTED_JSON")
    validated_severity=$(jq -r '.benchmark_contract.passrole_lambda.validated_severity' "$EXPECTED_JSON")
    ...
    validated_count=$(jq --arg src "$source_arn" --arg tgt "$target_arn" --arg severity "$validated_severity" '[.findings[] | select(.pattern_id == "passrole_lambda" and .source.provider_id == $src and .target.provider_id == $tgt and .verdict == "validated" and .severity == $severity)] | length' "$FINDINGS_JSON")
    ...
    precondition_only_count=$(jq --arg src "$source_arn" --arg tgt "$target_arn" '[.findings[] | select(.pattern_id == "passrole_lambda" and .source.provider_id == $src and .target.provider_id == $tgt and .verdict == "precondition_only")] | length' "$FINDINGS_JSON")
    ...
    required_pass_check_count=$(jq --arg src "$source_arn" --arg tgt "$target_arn" '[.findings[] | select(
        ...
        (.name == "source_has_lambda_create_function" or .name == "source_has_passrole_to_target" or .name == "target_trusts_lambda_service")
        and .state == "pass"
      )] | length' "$FINDINGS_JSON")

    echo "semantic assertion counts: scenario.lambda_create_edges=$lambda_create_edge_count scenario.passrole_edges=$passrole_edge_count scenario.lambda_trust_edges=$lambda_trust_edge_count passrole_lambda.validated=$validated_count passrole_lambda.blocked=$blocked_count passrole_lambda.inconclusive=$inconclusive_count passrole_lambda.precondition_only=$precondition_only_count validated_with_blockers=$validated_with_blockers_count required_pass_checks=$required_pass_check_count scenario.validation=$scenario_validation_ok" | tee -a "$RUN_LOG"

    if (( lambda_create_edge_count >= min_edge_count && passrole_edge_count >= min_edge_count && lambda_trust_edge_count >= min_edge_count && validated_count >= validated_min && blocked_count == blocked_expected && inconclusive_count == inconclusive_expected && precondition_only_count == precondition_only_expected && validated_with_blockers_count == validated_with_blockers_expected && required_pass_check_count >= 3 )); then
...
cat <<EOF
Env18 benchmark pass finished with command exit code: $cmd_rc
Env18 benchmark final result code: $benchmark_rc
