...
ENV_DIR="$PROJECT_ROOT/acceptance/env09_env05_boundary_removed"
EXPECTED_JSON="$ENV_DIR/expected_findings.json"
RUN_ID="${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)}"
WORK_DIR="${IAMSCOPE_ENV09_WORKDIR:-/tmp/iamscope-env09-benchmark-$RUN_ID}"
ARTIFACT_DIR="${IAMSCOPE_BENCHMARK_OUT:-/tmp/iamscope-benchmark-env09-$RUN_ID}"
...
if [[ "$raw_rc" =~ ^[0-9]+$ ]]; then
  ...
  echo "WARNING: non-numeric exit code from Env09 run: [$raw_rc]" | tee -a "$RUN_LOG" >&2
fi
...
if [[ ! -f "$FINDINGS_JSON" ]]; then
  ...
  if [[ -z "$source_arn" || -z "$target_arn" ]]; then
    ...
    echo "semantic assertion counts: ${target_pattern}.validated=$validated_count ${target_pattern}.blocked=$blocked_count ${target_pattern}.inconclusive=$inconclusive_count validated.blockers=$blocker_count assume_role_chain.validated=$arc_validated_count assume_role_chain.blocked=$arc_blocked_count scenario.validation=$scenario_validation_ok" | tee -a "$RUN_LOG"
...
cat <<EOF
Env09 benchmark pass finished with command exit code: $cmd_rc
Env09 benchmark final result code: $benchmark_rc
